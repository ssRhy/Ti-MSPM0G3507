...
void MPU6050_Init(void)
{
    ...
    if (result) {
        // 初始化失败
        char buf[32];
        sprintf(buf, "mpu_init failed: %d\r\n", result);
        uart0_send_string(buf);
        return;
    }
    uart0_send_string("mpu_init OK\r\n");
    ...
    if (result) {
        // DMP初始化失败，不重启芯片，让程序继续运行
        return;
    }

    /* Enable INT handler if needed - using SysConfig GPIO interrupt instead */
}
...
int Read_Quad(void)
{
    ...
    // 先检查 FIFO 是否有数据
    unsigned char tmp[2];
    result = i2c_read(0x68, 0x3A, 1, tmp);  // 读取 INT_STATUS 寄存器
    if(result != 0)
        return -1;

    // 检查 DMP 是否准备好
    if(!(tmp[0] & 0x01))  // 检查 DMP_INT_STATUS 位
        return -1;
...
// 读取原始传感器数据（不依赖 DMP）
int MPU6050_Read_Raw_Data(short *accel_data, short *gyro_data)
{
    unsigned char raw_data[14];

    // 读取加速度计和温度计以及陀螺仪数据
    if(i2c_read(0x68, 0x3B, 14, raw_data) != 0)
        return -1;

    // 合成为 short（MSP430/MSPM0 是小端模式）
    accel_data[0] = (raw_data[0] << 8) | raw_data[1];
    accel_data[1] = (raw_data[2] << 8) | raw_data[3];
    accel_data[2] = (raw_data[4] << 8) | raw_data[5];

    gyro_data[0] = (raw_data[8] << 8) | raw_data[9];
    gyro_data[1] = (raw_data[10] << 8) | raw_data[11];
    gyro_data[2] = (raw_data[12] << 8) | raw_data[13];
